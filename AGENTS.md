...
- **`romof` vs `cloneof` in `resolve_parents`**: `romof` is the split parent — it tells which zip holds shared ROMs (can chain, e.g. `3countb → neogeo`). `cloneof` is clone-relationship metadata (e.g. `2020bbh` is a hack of `2020bb`). `parent_game_id` is set from `cloneof` only; `romof` is NOT stored in `parent_game_id` because it's per-version (`game_rom_sets`) while `parent_game_id` is cross-version (`games`). `romof` is resolved at build time by `compute_game_roms` via `db.get_romof()`.
