...
- **`romof` vs `cloneof` in `resolve_parents`**: `romof` is the split parent — it tells which zip holds shared ROMs (can chain, e.g. `3countb → neogeo`). `cloneof` is clone-relationship metadata (e.g. `2020bbh` is a hack of `2020bb`). `parent_game_id` is set from `romof` first; `cloneof` is only used as fallback when `romof` is unavailable. Priority order: `romof` → `cloneof` → none.
